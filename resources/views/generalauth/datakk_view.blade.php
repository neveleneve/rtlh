                    ...
                    <div class="row">
                        ...
                        <div class="col-12 col-lg-6 mb-3">
                            ...
                            <a href="{{ asset('images/kk/' . $data[0]['no_kk'] . '.jpg') }}" data-toggle="lightbox">
                                <img src="{{ asset('images/kk/' . $data[0]['no_kk'] . '.jpg') }}"
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <p class="form-control">
                                {{ $data[0]['status'] == 0 ? 'Belum Verifikasi' : 'Sudah Verifikasi' }}
                            </p>
                        </div>
                        @if (Auth::user()->level == 1)
                            ...
                            @forelse ($nilai_pembobotan as $item)
                                <div class="col-12 mb-3">
                                    <label for="{{ $item->id_nama }}">{{ ucwords($item->nama) }} (Bobot :
                                        {{ $item->bobot }} Poin)</label>
                                    <input class="form-control" type="text" name="{{ $item->id_nama }}"
                                        id="{{ $item->id_nama }}" value="{{ $item->nilai }}" readonly>
                                </div>
                            @empty
                            @endforelse
                        @endif
