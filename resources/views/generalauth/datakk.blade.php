                                    ...
                                    @if (Auth::user()->level == 0)
                                        <div class="col-12 col-lg-4 mb-3">
                                            ...
                                            <select id="kotakab" name="kotakab" class="form-control form-control-sm">
                                                <option value="all"
                                                    {{ isset($_GET['kotakab']) ? ($_GET['kotakab'] == 'all' ? 'selected' : null) : 'selected' }}>
                                                    Semua</option>
                                                @foreach ($datakotakab as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ isset($_GET['kotakab']) ? ($_GET['kotakab'] == $item->id ? 'selected' : null) : null }}>
                                                        {{ ucwords(strtolower($item->name)) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-12 col-lg-4 mb-3">
                                            ...
                                            <select id="kecamatan" name="kecamatan" class="form-control form-control-sm">
                                                <option value="all"
                                                    {{ isset($_GET['kecamatan']) ? ($_GET['kecamatan'] == 'all' ? 'selected' : null) : 'selected' }}>
                                                    Semua</option>
                                                @foreach ($datakecamatan as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ isset($_GET['kecamatan']) ? ($_GET['kecamatan'] == $item->id ? 'selected' : null) : null }}>
                                                        {{ ucwords(strtolower($item->name)) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-12 col-lg-4 mb-3">
                                            ...
                                            <select id="kelurahan" name="kelurahan" class="form-control form-control-sm">
                                                <option value="all"
                                                    {{ isset($_GET['kelurahan']) ? ($_GET['kelurahan'] == 'all' ? 'selected' : null) : 'selected' }}>
                                                    Semua</option>
                                                @foreach ($datakelurahan as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ isset($_GET['kelurahan']) ? ($_GET['kelurahan'] == $item->id ? 'selected' : null) : null }}>
                                                        {{ ucwords(strtolower($item->name)) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                ...
                <div class="table-responsive">
                    <table class="table border table-hover text-center">
                        ...
                        <tbody class="align-middle">
                                ...
                                <tr>
                                    <td>
                                        <a href="{{ route('viewdatakk', ['id' => $item->no_kk]) }}"
                                            class="badge badge-sm bg-info text-white">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                    </td>
                                    <td>{{ $item->no_kk }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ Auth::user()->level == 0 ? '8' : '7' }}">
        ...
        @if ($data->links())
            <div class="row mb-3" id="pagination_info">
                <div class="col-12 text-center">
                    @if (count($data) > 0)
                        <small>Menampilkan {{ ($data->currentPage() - 1) * $data->perPage() + 1 }} -
                            {{ $data->currentPage() == $data->lastPage() ? $data->total() : $data->currentPage() * $data->perPage() }}
                            dari {{ $data->total() }} data</small>
                    @endif
                </div>
            </div>
        @endif
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('adddatakk') }}" method="post" enctype="multipart/form-data">
                            {{ csrf_field() }}
                            <label for="tahunanggaran">Tahun Anggaran<span class="text-danger">*</span></label>
                            <select id="tahunanggaran" name="tahunanggaran" class="form-control form-control-sm">
                                <option value="" disabled selected hidden>Pilih Tahun Anggaran</option>
                                @for ($i = 0; $i < 3; $i++)
                            ...
                            <div class="row justify-content-center">
                                <div class="col-md-12 text-center mb-2">
                                    <img id="preview-image-kk" src="{{ asset('images/template/not-found.png') }}"
                            ...
                            <div class="row justify-content-center">
                                <div class="col-md-12 text-center mb-2">
                                    <img id="preview-image-ktp" src="{{ asset('images/template/not-found.png') }}"
                                        style="max-height: 250px;">
                                    <br>
                            ...
                            @forelse ($pembobotan as $item)
                                <div class="col-12 mb-3">
                                    ...
                                    <select name="nilai[{{ $item->id }}]" id="{{ $item->id_nama }}"
                                        class="form-control" required>
                                        <option value="" disabled selected hidden>Pilih {{ ucwords($item->nama) }}
                                        </option>
                                        @forelse ($nilai_pembobotan[$item->id - 1] as $dataitem)
                                            <option value="{{ $dataitem->value }}">{{ ucwords($dataitem->nama) }}
                                            </option>
                                        @empty
                                        @endforelse
                                    </select>
...
@section('customjs')
    ...
    <script type="text/javascript" src="{{ route('scripting', ['filename' => 'datakk.js']) }}"></script>
@endsection
