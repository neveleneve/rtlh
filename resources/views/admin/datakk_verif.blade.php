            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-lg-6 mb-3">
                        ...
                        <br>
                        <a href="{{ asset('images/ktp/' . $data[0]['nik'] . '.jpg') }}" id="fotonik"
                            data-toggle="lightbox">
                            <img src="{{ asset('images/ktp/' . $data[0]['nik'] . '.jpg') }}"
                    ...
                    <form action="{{ route('verifikasi') }}" method="post">
                        ...
                        @forelse ($nilai_pembobotan as $item)
                            <div class="col-12 mb-3">
                                <label for="{{ $item->id_nama }}">{{ ucwords($item->nama_kriteria) }} (Bobot :
                                    {{ $item->bobot }} Poin)</label>
                                <input class="form-control" type="text" name="{{ $item->id_nama }}"
                                    id="{{ $item->id_nama }}"
                                    value="{{ ucwords(strtolower($item->nama_nilai)) }} (Nilai : {{ $item->nilai }} poin)"
                                    readonly>
                            </div>
                        @empty
                        @endforelse
                        <div class="col-12 mb-3">
                            ...
                            <input class="form-control" type="text" name="hasil" id="hasil"
                                value="{{ $hasil_pembobotan }}" readonly>
                        </div>
                        <div class="col-12 d-grid gap-2">
                            <button class="btn btn-dark" type="submit"
                                onclick="return confirm('Verifikasi data pengaju dengan nomor KK : '+ {{ $data[0]['no_kk'] }})">
